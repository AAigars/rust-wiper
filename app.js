...
const pterodactyl = new (require("./pterodactyl"))(config.PTERODACTYL.URL, config.PTERODACTYL.API_KEY);
...
const wipeServer = async (wipe) => {
    ...
    const files = await pterodactyl.getFiles(wipe.PTERODACTYL_ID, `/server/${wipe.SERVER_IDENTITY}`);
    files.data.forEach(async file => {
        const name = file.attributes.name;
        if(name == "cfg" || name == "companion.id" || name.includes("player.blueprints")) return; 

        await pterodactyl.deleteFile(wipe.PTERODACTYL_ID, `/server/${wipe.SERVER_IDENTITY}`, name);
        console.log(wipe.SERVER_NAME + ` | Wiped /server/${wipe.SERVER_IDENTITY}/${name}`);
    });
    ...
    discord.sendNotification(wipe.SERVER_NAME, wipe.SERVER_IP, seed, wipe.SERVER_SIZE, "Map Wipe", mapThumbnail);
};

config.WIPES.forEach(wipe => {
    new CronJob(wipe.TIME, () => wipeServer(wipe), null, null, wipe.TIMEZONE).start();
});